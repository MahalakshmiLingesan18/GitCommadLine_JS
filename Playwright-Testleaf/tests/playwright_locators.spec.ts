...
const fName = "Mahalakshmi";
const lName = "Lingesan";
const companyName = "WPP";
const companyName2 = "Business";
...
test("To edit a lead", async({page}) => {
    ...
    await page.fill("#updateLeadForm_departmentName", "Fashion Technology");
    await page.fill("//textarea[@name='description']", "I am a Business entrepreneur");
    ...
    expect.soft(department).toMatch("Fashion Technology");
    console.log(`Last name: ${department}`);

    const description = await page.innerText("#viewLead_description_sp");
    expect.soft(description).toEqual("I am a Business entrepreneur");
